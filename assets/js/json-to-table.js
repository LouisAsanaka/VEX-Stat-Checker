...
function ConvertJsonToTable(parsedJson, tableId, tableClassName, linkText)
{
    ...
    var italic = '<i>{0}</i>';
    var link = linkText ? '<a href="https://www.robotevents.com/robot-competitions/vex-robotics-competition/{0}.html">' + linkText + '</a>' :
                          '<a href="https://www.robotevents.com/robot-competitions/vex-robotics-competition/{0}.html">{0}</a>';
            ...
            if(headers)
            {
                ...
                for (i = 0; i < parsedJson.length; i++)
                {
                    for (j = 0; j < headers.length; j++)
                    {
						...
						console.log(value);
                        if(isUrl || (typeof value === "string" && value.indexOf("RE-") !== -1))   // If value is URL we auto-create a link
